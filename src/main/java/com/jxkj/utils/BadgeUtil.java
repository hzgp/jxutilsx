...
/**
 * Created by DY on 2020/1/6.
 */

public class BadgeUtil {
    public static final String BADGE_CHANNEL_ID = "user.badge";
    public static final int notificationId = 12;
    public static int MIUI_VERSION=-1;
     ...
     * 设置MIUI的Badge
     */
    public static void setBadgeOfMIUI(Context context, int count, int iconResId) {
        Notification notification = null;
        int minuVesion = getMIUIVersion();
        if (minuVesion > 0) {
            if (minuVesion >= 6 && minuVesion <= 11) {
                try {
                    notification = createNotify(context, BADGE_CHANNEL_ID, iconResId, count);
                    Field field = notification.getClass().getDeclaredField("extraNotification");
                    Object extraNotification = field.get(notification);
                    Method setCountMethod = extraNotification.getClass().getDeclaredMethod("setMessageCount", Integer.class);
                    setCountMethod.invoke(extraNotification, notificationId);
                } catch (Exception e) {
                    CrashHandler.getInstance().saveCrashInfoToFile(e);
                }
            } else if (minuVesion >= 12) {
                notification = createNotify(context, BADGE_CHANNEL_ID, iconResId, count);
            }
        }
        if (notification != null) NotificationManagerCompat.from(context).notify(notificationId, notification);
    }

    private static int getMIUIVersion() {
        if (MIUI_VERSION!=-1)return MIUI_VERSION;
        String systemProperties = "android.os.SystemProperties";
        Class<?> cls;
        Object property = null;
        try {
            cls = Class.forName(systemProperties);
            Method method = cls.getMethod("get", String.class, String.class);
            property = method.invoke(cls, "ro.miui.ui.version.code", "7");
        } catch (Exception e) {
            CrashHandler.getInstance().saveCrashInfoToFile(e);
        }
        if (property == null) return -1;
        return property instanceof String ? Integer.parseInt((String) property) : -1;
    }

    private static Notification createNotify(Context context, String channelId, int img, int count) {
        Activity main = AppManager.getAppManager().getMainActivity();
        ...
        if (main!=null){
            intent.setComponent(new ComponentName(context,main.getClass()));
        }else {
            intent.setComponent(new ComponentName(context,context.getPackageName()+".ui.activity.MainActivity"));
        }
        pendingIntent = PendingIntent.getActivity(context,123,intent,FLAG_UPDATE_CURRENT);
